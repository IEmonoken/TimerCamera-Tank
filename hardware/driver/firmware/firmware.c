...
// CONFIG1
#pragma config FOSC = INTOSC    // 内部オシレーター使用 CLKINピンはデジタルピン
...
#pragma config BOREN = ON       // 電源電圧がVBORより下がったらリセット
#pragma config CLKOUTEN = OFF   // クロックを外部に出力しない。
#pragma config IESO = OFF       // 外部クロックなし。クロック切り替え無効
#pragma config FCMEN = OFF      // 外部クロックなし。監視しない。

// CONFIG2
#pragma config WRT = OFF        // フラッシュメモリ 書き込みプロテクト無効
#pragma config PLLEN = OFF      // PLLは無効
#pragma config STVREN = ON      // スタックオーバーフローやスタックアンダーフローでリセット
#pragma config BORV = HI        // VBORVは2.5V
#pragma config LVP = OFF        // 低電圧プログラミングは行わない
...
#define AN0_EN      LATBbits.LATB0      // AN0は0で有効
// RB1はSDA
#define BAT_EN      LATBbits.LATB2      // 電源FET
